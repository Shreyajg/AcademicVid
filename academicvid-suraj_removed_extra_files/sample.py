...
def read_pdf(file):
    try:
        ...
        for page_num in range(len(pdf_reader.pages)):
            ...
            if extracted_text:  # Only concatenate if the extracted text is not None
                text += extracted_text
            else:
                text += ""  # Or handle it differently, like appending a note
        return text
    except Exception as e:
        print(f"Error: {e}")
        return "" 

def write_text_to_pdf(text, output_filename):
    try:
        ...
        for line in lines:
            if y < 50:  # Go to next page if at bottom
                c.showPage()
                y = height - 50
...
def main():
    ...
    st.sidebar.header("Enter Topic")
    st.subheader("Topic:")

    topic_input = st.sidebar.text_input("Enter the topic")

    if topic_input:
        st.sidebar.success(f"Topic: {topic_input} entered successfully!")
        # Fetch PDFs from the web based on the topic
        pdf_links = fetch_pdfs_from_web(topic_input)
        if pdf_links:
            st.sidebar.success(f"Found {len(pdf_links)} PDFs for the topic!")
            all_pdf_text = ""
            for pdf_link in pdf_links:
                pdf_text = extract_pdf_from_url(pdf_link)
                all_pdf_text += pdf_text
            
            if all_pdf_text:
                response = get_gemini_response(input_prompt1, all_pdf_text)
                # Process the rest as in the previous code
                write_text_to_pdf(response,f"{topic_input}.pdf")
        else:
            st.sidebar.warning("No PDFs found for this topic.")
...
def get_discription(image_bytes):
    ...
    response = model.generate_content(
        [
            "Explain image, make the response more educational type in english,contextual,brief,short in a way like how a teacher explains to a student in the class",
            img,
        ],
        stream=True,
    )
...
def get_image_prompt(image_url):
    ...
    response = model.generate_content(
        [
            "Explain image, make the response more educational type in english,contextual,brief,short in a way like how a teacher explains to a student in the class, in 2 to 3 lines including main keywords from the image",
            img,
        ],
        stream=True,
    )
