...
client = OpenAI(api_key="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")
input_prompt1 = """
...
def get_openai_response(input, pdf_content):
...
def main():
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Reading PDF..."):
            ...
            response = get_openai_response(input_prompt1, pdf_text)
            print(response)
            # old one
...
def clean_text(text: str) -> str:
    if not text:  # safeguard against None
        return ""

    # remove LaTeX expressions inside $...$ or \[...\]
    text = re.sub(r"\$.*?\$", "", text, flags=re.DOTALL)
    text = re.sub(r"\\\[.*?\\\]", "", text, flags=re.DOTALL)

    # optionally remove LaTeX commands like \frac, \begin{}, etc.
...
def information(data, imagemap, topic):
    ...
    if topic in imagemap:
        ...
        my_text_original = ""
        my_text_original = get_description(imagemap[topic])
        bad_chars = [";", ":", "!", "*", "#", "-"]
        for i in bad_chars:
...
def get_custom_search_results(query, search_type="image"):
    ...
    params = {
        "key": "AIzaSyAwnw0Hgi8kTKXSVZQ-xUyORQcRtvUntOo",  # keep your Google key
        "cx": "172574863a68442ad",  # your search engine ID
        "q": query,
    ...
    if response.status_code == 200:
        ...
        for item in items:
            try:
                url = item["link"]
                get_image_prompt(url)  # test if image works
...
def get_description(image_bytes):
    img_b64 = image_bytes  # keep as bytes
    response = client.chat.completions.create(
        model="gpt-4o-mini",  # lightweight model w/ vision
        messages=[
            {"role": "system", "content": "You are a teacher explaining this content to a student. Keep it short, simple, avoid LaTeX, just focus on the main idea."},
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "Explain this image:"},
                    {"type": "image_url", "image_url": {"url": "data:image/jpeg;base64," + img_b64.decode()}}
                ],
            },
        ],
    )
...
def map_image_heading(imagemap, image_bytes, heading):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": f"Pick ONE heading that best suits this image from: {','.join(heading)}"},
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "Pick one heading only."},
                    {"type": "image_url", "image_url": {"url": "data:image/jpeg;base64," + img_b64.decode()}}
                ],
            },
        ],
    )
    chosen_heading = response.choices[0].message.content
    imagemap[chosen_heading] = image_bytes
